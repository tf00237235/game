...
class Figlet
{
     ...
     * @var int
     */
    protected $outlineLengthLimit = 2;
    ...
    protected function _loadFont($fontFile)
    {
        ...
        if (!file_exists(__DIR__ . "\\" . $fontFile)) {
            throw new Exception\RuntimeException(__DIR__ . "\\" . $fontFile . ': Font file not found');
        }

        // Check if gzip support is required
        if (substr(__DIR__ . "\\" . $fontFile, -3) === '.gz') {
            if (!function_exists('gzcompress')) {
                throw new Exception\RuntimeException('GZIP library is required for gzip compressed font files');
            }

            $fontFile = 'compress.zlib://' . __DIR__ . "\\" . $fontFile;
            $compressed = true;
        } else {
            $compressed = false;
        }
        ...
        $fp = fopen(__DIR__ . "\\" . $fontFile, 'rb');
        if ($fp === false) {
            throw new Exception\RuntimeException(__DIR__ . "\\" . $fontFile . ': Could not open file');
        }
        ...
        if ($magic !== self::FONTFILE_MAGIC_NUMBER || $numsRead < 5) {
            throw new Exception\UnexpectedValueException(__DIR__ . "\\" . $fontFile . ': Not a FIGlet 2 font file');
        }
